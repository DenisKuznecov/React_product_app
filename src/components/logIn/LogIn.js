...
class LogIn extends React.Component {
	...
	getUserData() {
		...
		if(usernameField.value.length !== 0 && passwordField.value.length !== 0) {
			...
			api.post(`${ApiUrls.loginUrl}`, JSON.stringify(loginObj), (response) => {
				console.log(response);
					if(response.success === true) {
						this.setState({
							user: response,
							tooltipClass: ' tooltip-success',
							tooltipText: 'You have successfully loged in.'
						});
			...
			<div className="jumbotron">
				<div className="login-form-wrap">
					...
					<form>
						...
						<button type="button" onClick={this.getUserData} className="btn btn-primary">Log In</button>
